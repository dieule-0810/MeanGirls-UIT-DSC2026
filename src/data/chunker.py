"""
src/data/chunker.py - Bộ cắt đoạn văn bản pháp luật lai (Hybrid Legal Document Chunker)
...
Cách chạy:
    python -m src.data.chunker --input data/corpus_clean.jsonl --out data/chunks.jsonl --chunk-size 256 --overlap 64
...
def sliding_window_with_dieu_tag(rc: str, chunk_size: int, overlap: int) -> list[str]:
    ...
    dieu_label = dieu_match.group(0) if dieu_match else ""
    raw_subs = sliding_window(rc, chunk_size, overlap)
    tagged = []
    for i, sub in enumerate(raw_subs):
        if i == 0 or not dieu_label or sub.strip().startswith(dieu_label):
            tagged.append(sub)
        else:
            tagged.append(f"[{dieu_label} - tiếp theo] {sub}")
    return tagged
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=Path("data/chunks.jsonl"),
                    help="Đường dẫn file đầu ra chunks.jsonl")
